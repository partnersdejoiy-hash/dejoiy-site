...
function NeuralViz({ color }) {
  ...
  return (
    <svg viewBox="0 0 440 360" className="w-full h-full">
      ...
      {nodes.map((n, i) => (
        <g key={i}>
          <motion.circle cx={n.x} cy={n.y} r={16} fill={`${color}08`} stroke={`${color}20`} strokeWidth="1"
            animate={{ scale: [1, 1.2, 1] }}
            transition={{ duration: 2, delay: i * 0.2, repeat: Infinity }}
          />
          <motion.circle cx={n.x} cy={n.y} r={5} fill={color}
...
function PipelineViz({ color }) {
  ...
  return (
    <svg viewBox="0 0 440 360" className="w-full h-full">
      <motion.line x1="40" y1="180" x2="400" y2="180" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
      {bars.map((h, i) => (
        <g key={i}>
          <motion.rect
            x={40 + i * 42} y={180 - h * 1.4} width={28} height={h * 1.4}
            rx="4"
            fill={`${color}${i % 2 === 0 ? "60" : "30"}`}
            stroke={`${color}50`} strokeWidth="1"
            initial={{ height: 0, y: 180 }}
            animate={{ height: h * 1.4, y: 180 - h * 1.4 }}
            transition={{ delay: i * 0.08, duration: 0.8, ease: [0.16,1,0.3,1], repeat: Infinity, repeatDelay: 3 }}
          />
          <motion.circle
            cx={40 + i * 42 + 14} cy={180 - h * 1.4 - 8} r={3}
            fill={color}
      ...
      <g transform="translate(220,180)">
        {rings.map((r, i) => (
          <motion.circle key={i} cx={0} cy={0} r={r}
            fill="none" stroke={`${color}${i === 0 ? "10" : i === 1 ? "18" : i === 2 ? "28" : "50"}`}
            strokeWidth={i === rings.length-1 ? 2 : 1}
            strokeDasharray={i % 2 === 0 ? "6 3" : "none"}
            initial={{ scale: 0, opacity: 0 }}
            animate={{ scale: 1, opacity: 1, rotate: i % 2 === 0 ? 360 : -360 }}
            transition={{ scale: { delay: 0.3 + i * 0.15, duration: 0.6, ease: [0.16,1,0.3,1] }, rotate: { duration: 20 - i * 3, repeat: Infinity, ease: "linear" }, opacity: { delay: 0.3 + i * 0.15, duration: 0.5 } }}
...
function WorkflowViz({ color }) {
  ...
  return (
    <svg viewBox="0 0 440 360" className="w-full h-full">
      {edges.map(([a,b],i) => (
        <motion.line key={i}
          x1={tasks[a].x} y1={tasks[a].y} x2={tasks[b].x} y2={tasks[b].y}
      ...
      <motion.circle
        ...
        animate={{ r: [40, 80, 40], opacity: [0.3, 0, 0.3] }}
